...
class ResidualSequentialNetwork(
    ...
    override fun predict(input: Tensor): Tensor {
        ...
        val activation1 = hidden1.activation.calculate(preactivation1)

        val preactivation2 = hidden2.forward(activation1)
        val activation2 = hidden2.activation.calculate(preactivation2)

        val preactivation3 = hidden3.forward(activation2)
        val activation3 = hidden3.activation.calculate(preactivation3)
        ...
        val outputActivation = output.activation.calculate(outputPreactivation)
    ...
    override fun forward(
        ...
        derivativeActivations[0] += hidden1.activation.derivative(preactivation1)

        val preactivation2 = hidden2.forward(activation1)
        val activation2 = hidden2.activation.calculate(preactivation2)
        activations[1] += activation2
        derivativeActivations[1] += hidden1.activation.derivative(preactivation2)

        val preactivation3 = hidden3.forward(activation2)
        val activation3 = hidden3.activation.calculate(preactivation3)
        activations[2] += activation3
        derivativeActivations[2] += hidden1.activation.derivative(preactivation3)

        val residualSum = activation1 + activation3
        val normalizedOutput = layerNorm(residualSum)
        activations[3] += normalizedOutput
        derivativeActivations[3] += hidden1.activation.derivative(normalizedOutput)

        val outputPreactivation = output.forward(normalizedOutput)
        val outputActivation = output.activation.calculate(outputPreactivation)
        activations[4] += outputActivation
        derivativeActivations[4] += hidden1.activation.derivative(outputActivation)
    }
